...
export default function Chats({userID_, globalWs, swapChattingTo}) {
    ...
    let rawContacts
        function switchFocusedChat(ind, contacts){
            const mappedContacts = contacts.map((contact, index) => {
                return (<div className={`flex items-center w-full p-32 mb-32 hover:bg-primary/[.2] rounded-md cursor-pointer ${ind !== index ? 'bg-inputBg' : 'bg-primary/[.4]'}`} key = {index} onClick = {() => {swapChattingTo(contact); switchFocusedChat(index, contacts)}}> 
                        <img src = {`/avatars/${contact.profile_img}`} className='inline-block w-40 h-40 mr-24'/> {contact.firstname} {contact.surname}
                    </div>)
            })
    ...
    useEffect(() => {
        ...
        //gets the contacts for the userID_
        async function  getContacts() {
            ...
            //sometimes the perspective changes
            ...
            const mappedContacts = contacts.map((contact, index) => {
                return (
                    <div className="w-full flex items-center  p-32 mb-32 hover:bg-primary/[.2] rounded-md cursor-pointer bg-inputBg" key = {index} onClick= {() => {swapChattingTo(contact); switchFocusedChat(index, contacts)}}>
